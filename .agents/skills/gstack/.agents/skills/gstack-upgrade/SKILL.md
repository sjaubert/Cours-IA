...
[ -z "$_AUTO" ] && _AUTO=$(./.agents/skills/gstack/bin/gstack-config get auto_upgrade 2>/dev/null || true)
...
```bash
./.agents/skills/gstack/bin/gstack-config set auto_upgrade true
```
...
**If "Never ask again":**
```bash
./.agents/skills/gstack/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `./.agents/skills/gstack/bin/gstack-config set update_check true` to re-enable."
...
```bash
./.agents/skills/gstack/bin/gstack-update-check --force 2>/dev/null || \
