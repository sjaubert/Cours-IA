...
```bash
_UPD=$(./.agents/skills/gstack/bin/gstack-update-check 2>/dev/null || .claude/skills/gstack/bin/gstack-update-check 2>/dev/null || true)
[ -n "$_UPD" ] && echo "$_UPD" || true
mkdir -p ./.gstack/sessions
touch ./.gstack/sessions/"$PPID"
_SESSIONS=$(find ./.gstack/sessions -mmin -120 -type f 2>/dev/null | wc -l | tr -d ' ')
find ./.gstack/sessions -mmin +120 -type f -delete 2>/dev/null || true
_CONTRIB=$(./.agents/skills/gstack/bin/gstack-config get gstack_contributor 2>/dev/null || true)
_PROACTIVE=$(./.agents/skills/gstack/bin/gstack-config get proactive 2>/dev/null || echo "true")
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
echo "PROACTIVE: $_PROACTIVE"
source <(./.agents/skills/gstack/bin/gstack-repo-mode 2>/dev/null) || true
REPO_MODE=${REPO_MODE:-unknown}
echo "REPO_MODE: $REPO_MODE"
_LAKE_SEEN=$([ -f ./.gstack/.completeness-intro-seen ] && echo "yes" || echo "no")
echo "LAKE_INTRO: $_LAKE_SEEN"
_TEL=$(./.agents/skills/gstack/bin/gstack-config get telemetry 2>/dev/null || true)
...
for _PF in ./.gstack/analytics/.pending-*; do [ -f "$_PF" ] && ./.agents/skills/gstack/bin/gstack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true; break; done
```
...
If A: run `./.agents/skills/gstack/bin/gstack-config set telemetry community`
...
If B→A: run `./.agents/skills/gstack/bin/gstack-config set telemetry anonymous`
If B→B: run `./.agents/skills/gstack/bin/gstack-config set telemetry off`
...
1. **Check for a prior override on this branch:**
   ```bash
   source <(./.agents/skills/gstack/bin/gstack-slug 2>/dev/null)
...
2. **If no override exists,** use AskUserQuestion:
   ...
   - For Design Review: run `source <(./.agents/skills/gstack/bin/gstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
```bash
source <(./.agents/skills/gstack/bin/gstack-slug 2>/dev/null) && mkdir -p ./.gstack/projects/$SLUG
USER=$(whoami)
...
```bash
source <(./.agents/skills/gstack/bin/gstack-diff-scope <base> 2>/dev/null)
```
...
```bash
./.agents/skills/gstack/bin/gstack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
# Respect old opt-out
OLD_CFG=$(./.agents/skills/gstack/bin/gstack-config get codex_reviews 2>/dev/null || true)
...
```bash
./.agents/skills/gstack/bin/gstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"medium","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
./.agents/skills/gstack/bin/gstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"large","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
