...
```bash
./.agents/skills/gstack/bin/gstack-review-read 2>/dev/null
```
...
```bash
eval $(./.agents/skills/gstack/bin/gstack-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
